...
export default function MapClient() {
  ...
  useEffect(() => {
    setVehicles(Array.from({length:6},(_,i)=>({
      id:i,name:`Bike ${i+1}`,zone:pick(),dest:pick(),col:COLORS[i],d:0,f:0,x:rnd(20,80),y:rnd(20,80),tx:rnd(20,80),ty:rnd(20,80)
    })));
  },[]);
  ...
  useEffect(()=>{
    ...
    const iv = setInterval(() => {
      setVehicles(p => p.map(b => {
        let nx = b.x + (b.tx - b.x) * 0.12;
        let ny = b.y + (b.ty - b.y) * 0.12;
  ...
  <main className="min-h-screen bg-gray-950 text-white flex flex-col">
    ...
    <div className="flex flex-1 overflow-hidden">
      ...
      <div className="flex-1 relative bg-gray-900 overflow-hidden">
        ...
        {/* Lagos label */}
        <div className="absolute top-6 left-8 text-5xl font-black text-white/5 select-none pointer-events-none tracking-widest">
          LAGOS CITY MAP
        </div>

        {/* Zone labels */}
        {ZONES.slice(0,12).map((z,i)=>(
          <div key={z} className="absolute text-[10px] text-gray-600 font-mono select-none pointer-events-none uppercase" style={{left:`${(i%5)*18+8}%`,top:`${Math.floor(i/5)*35+10}%`}}>
            {z}
          </div>
        ))}
          ...
          <div key={b.id} className="transition-all duration-[1000ms] ease-linear absolute z-10" style={{left:`${b.x}%`,top:`${b.y}%`,transform:"translate(-50%,-50%"}}>
            ...
            {on && (
              <svg style={{position:"absolute",top:-30,left:-60,width:120,height:120,pointerEvents:"none",overflow:"visible"}}>
                <line 
                  x1="60" y1="60" 
                  x2={`${b.tx>60?-40:160}`} y2={`${b.ty>60?-40:160}`}
                  stroke={b.col} strokeWidth="2" strokeDasharray="4,4" opacity=".7"
                />
              </svg>
            )}
          </div>
      ...
      <div className="w-72 border-l border-gray-800 bg-gray-900 p-4 flex flex-col gap-4 overflow-y-auto">
        ...
        <div className="grid grid-cols-2 gap-3">
          <div className="bg-gray-800 rounded-xl p-3 text-center">
            <div className="text-green-400 font-bold text-xl">{del}</div>
            <div className="text-gray-500 text-xs">Deliveries</div>
          </div>
          <div className="bg-gray-800 rounded-xl p-3 text-center">
            <div className="text-orange-400 font-bold text-xl">₦{fuel}</div>
        ...
        <div className="bg-orange-500/10 border border-orange-500/20 rounded-xl p-3 mt-auto">
          <div className="flex items-center gap-2 text-orange-400 text-xs font-semibold mb-2">
            <Zap size={12} /> A* Algorithm Active
          </div>
          <p className="text-gray-400 text-xs leading-relaxed">
            Real-time multi-vehicle routing with dynamic traffic weights & order clustering optimization. O(n log n) performance.
          </p>
        </div>
