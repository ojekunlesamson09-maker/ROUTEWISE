  ...
  useEffect(() => {
    ...
    import("leaflet").then((L) => {
      ...
      vehicles.forEach((v) => {
        ...
        if (simulating) {
          const line = L.polyline([v.position, v.target], {
            color: v.color,
            weight: 2,
            opacity: 0.6,
            dashArray: "6, 6",
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      setVehicles((prev) =>
        prev.map((v) => {
          const newLat =
            v.position[0] + (v.target[0] - v.position[0]) * 0.2;
          const newLng =
            v.position[1] + (v.target[1] - v.position[1]) * 0.2;
          const dist =
            Math.abs(newLat - v.target[0]) +
            Math.abs(newLng - v.target[1]);
        ...
        <div className="w-72 border-l border-gray-800 bg-gray-900 p-4 flex flex-col gap-4 overflow-y-auto">
          ...
          <div className="bg-orange-500/10 border border-orange-500/20 rounded-xl p-3">
            ...
            <p className="text-gray-400 text-xs leading-relaxed">
              Each vehicle follows an A* optimized path with dynamic traffic
              weights. Routes are recalculated every 2s as new orders arrive.
            </p>
          </div>
        </div>
      </div>
